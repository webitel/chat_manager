...
var psqlInternalSessionQ = CompactSQL(
	`WITH session AS (select id
     ...
     channel as (select chat.domain_id                                          as dc,
                        ...
                        coalesce(chat.closed_at, channel.closed_at)             as closed_at
                 from chat.conversation as channel
                          join chat.channel as chat
                               on chat.conversation_id = channel.id and chat.connection similar to '\\d+' and
                                  chat.created_at <= channel.created_at + '3 millisecond'
                          left join chat.bot on (chat.connection::int8) = bot.id
                          join flow.acr_routing_scheme flow
                               ON flow.id = (channel.props ->> 'flow')::int8 and flow.domain_id = channel.domain_id
                          left join chat.conversation_node as service on channel.id = service.conversation_id
                 where channel.id = (select id from session)
                 union all
                 select channel.domain_id                                                           as dc,
                        channel.conversation_id                                                     as room_id,
                        channel.id                                                                  as chat_id,
                        coalesce(nullif(channel.type, 'webitel'), 'websocket')                      as chat_channel,
                        coalesce(channel.connection || coalesce('@' || channel.host, ''), 'engine') as chat_contact,
                        ...
                        channel.closed_at
                 from chat.channel
                          left join chat.client as contact on not (channel.internal) and channel.user_id = contact.id
                          left join directory.wbt_user as account on channel.internal and channel.user_id = account.id
                 where channel.conversation_id = (select id from session)
                 union all
                 select invite.domain_id                         as dc,
                        invite.conversation_id                   as room_id,
                        invite.id                                as chat_id,
                        'websocket'                              as chat_channel,
                        'engine'                                 as chat_contact,
                        ...
                        null                                     as closed_at
                 from chat.invite
                          left join directory.wbt_user as account on invite.user_id = account.id
                 where invite.conversation_id = (select id from session)
                   and invite.closed_at isnull)
select chat.dc,
       chat.room_id,
       chat.chat_id,
       chat.chat_channel,
