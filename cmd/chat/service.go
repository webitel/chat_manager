...
func (c *chatService) BroadcastMessage(ctx context.Context, req *pbmessages.BroadcastMessageRequest, resp *pbmessages.BroadcastMessageResponse) error {
	...
	// NOTE: Transform requests to correct structs
	toBotsService := transformToBotsServiceRequest(req)
	toChatMessagesService := transformToChatMessagesServiceRequest(req)

	var errors []*pbmessages.BroadcastError

	// NOTE: Forward requests to BotsService
	errors = c.executeBotsService(toBotsService)
	if len(errors) > 0 {
		resp.Failure = append(resp.Failure, errors...)
	}

	// NOTE: Forward requests to ChatMessagesService
	errors = c.executeChatMessagesService(toChatMessagesService)
	if len(errors) > 0 {
		resp.Failure = append(resp.Failure, errors...)
	}
...
func transformToBotsServiceRequest(inbound *pbmessages.BroadcastMessageRequest) []*pbbot.BroadcastMessageRequest {
	...
	dtos := make(map[int64]*pbbot.BroadcastMessageRequest)
	for _, peer := range inbound.GetPeers() {
		switch peer.GetType() {
		case "telegram", "viber", "facebook", "messenger", "instagram", "whatsapp", "vk":
			...
			dto, ok := dtos[parsedVia]
			if !ok {
				dto = &pbbot.BroadcastMessageRequest{
					From:    parsedVia,
					Message: chatMessage,
				}
			}

			dto.Peer = append(dto.Peer, peer.GetId())
			dtos[parsedVia] = dto
		}
	}

	// NOTE: Init outbounds array with DTOs
	outbounds := make([]*pbbot.BroadcastMessageRequest, 0, len(dtos))
	for _, dto := range dtos {
		outbounds = append(outbounds, dto)
	}

	return outbounds
}

func transformToChatMessagesServiceRequest(inbound *pbmessages.BroadcastMessageRequest) *pbmessages.BroadcastMessageRequest {
...
func (c *chatService) executeBotsService(reqs []*pbbot.BroadcastMessageRequest) []*pbmessages.BroadcastError {
	var errors []*pbmessages.BroadcastError

	for _, req := range reqs {
		if len(req.GetPeer()) == 0 || req.GetFrom() == 0 {
			continue
		}
		...
		if err != nil {
			...
			resp.Variables = resp.GetVariables()
		}
	}
