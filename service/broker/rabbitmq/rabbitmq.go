...
// Source https://github.com/go-micro/plugins/tree/main/v4/broker/rabbitmq
package rabbitmq
...
func (r *rbroker) Subscribe(topic string, handler broker.Handler, opts ...broker.SubscribeOption) (broker.Subscriber, error) {
	...
	fn := func(msg amqp.Delivery) {
		...
		header["timestamp"] = msg.Timestamp.Format(time.RFC3339Nano)
		...
		p.err = handler(m)                          // handler(p)
		if p.err == nil && ackSuccess && !autoAck { // !opt.AutoAck {
			msg.Ack(false)
		} else if p.err != nil && !autoAck { // !opt.AutoAck {
			msg.Nack(false, requeueOnError)
		}
	}

	sret := &subscriber{topic: topic, mayRun: true, autoAck: autoAck, opts: opt,
		r: r, durableQueue: durableQueue, fn: fn, headers: headers, queueArgs: qArgs}
